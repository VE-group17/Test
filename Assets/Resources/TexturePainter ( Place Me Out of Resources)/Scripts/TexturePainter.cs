...
public class TexturePainter : MonoBehaviour, IUseable {
	...
	bool HitTestUVPosition(ref Vector3 uvWorldPosition){
        ...
        GameObject temp2 = GameObject.Find("Cylinder");
        Transform top_sprayTransform = temp2.GetComponent<Transform>();

        //Vector3 cursorPos = new Vector3 (Input.mousePosition.x, Input.mousePosition.y, 0.0f);
        // Vector3 cursorPos = new Vector3 (0.0f, 0.0f, 0.0f);

        Vector3 local_target = new Vector3(0.0f, 0.0f, 10.0f) + top_sprayTransform.position;

        Vector3 global_target = sprayTransform.TransformPoint(local_target);
        Ray cursorRay = new Ray(sprayTransform.position, global_target - sprayTransform.position);
